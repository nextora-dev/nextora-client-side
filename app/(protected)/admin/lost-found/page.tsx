...
export default function AdminLostFoundPage() {
    ...
    const {
        ...
        claims, totalClaims, selectedClaim, stats,
    ...
    // 0=Lost, 1=Found, 2=Pending Claims, 3=Approved Claims, 4=Rejected Claims
    ...
    const handleOpenMenu = (event: React.MouseEvent<HTMLElement>, item: LostItemResponse | FoundItemResponse, type: 'LOST' | 'FOUND') => {
        setMenuAnchor(event.currentTarget); setMenuItem({ item, type });
    };
    const handleCloseMenu = () => { setMenuAnchor(null); setMenuItem(null); };

    const handleView = () => {
        if (!menuItem) return;
        if (menuItem.type === 'LOST') loadLostItemById(menuItem.item.id); else loadFoundItemById(menuItem.item.id);
        setDetailType(menuItem.type); setDetailOpen(true); handleCloseMenu();
    };

    const handleDeactivate = () => {
        if (!menuItem) return;
        setDeactivateTarget({ id: menuItem.item.id, title: menuItem.item.title, type: menuItem.type });
        setDeactivateOpen(true); handleCloseMenu();
    };

    const handleConfirmDeactivate = () => {
        if (!deactivateTarget) return;
        const promise = deactivateTarget.type === 'LOST'
            ? adminUpdateLost(deactivateTarget.id, { active: false })
            : adminUpdateFound(deactivateTarget.id, { active: false });
        promise.then(() => { setDeactivateOpen(false); setDeactivateTarget(null); reload(); loadAdminStats(); });
    };

    const handleDeleteClick = () => {
        if (!menuItem) return;
        setDeleteTarget({ id: menuItem.item.id, title: menuItem.item.title, type: menuItem.type });
        setDeleteOpen(true); handleCloseMenu();
    };

    const handleConfirmDelete = () => {
        if (!deleteTarget) return;
        const promise = deleteTarget.type === 'LOST' ? adminDeleteLost(deleteTarget.id) : adminDeleteFound(deleteTarget.id);
        promise.then(() => { setDeleteOpen(false); setDeleteTarget(null); reload(); loadAdminStats(); });
    };

    const handleApprove = (id: number) => { approveClaim(id).then(() => reload()); };
    const handleRejectOpen = (id: number) => { setRejectClaimId(id); setRejectOpen(true); };
    const handleRejectConfirm = (id: number, reason: string) => { rejectClaim(id, reason).then(() => { setRejectOpen(false); reload(); }); };
    ...
    return (
        <Box sx={{ maxWidth: 1400, mx: 'auto' }}>
            <Box sx={{ mb: 4 }}>
                <Typography variant="h4" fontWeight={700} sx={{ letterSpacing: '-0.02em' }}>Lost & Found Administration</Typography>
                <Typography variant="body2" color="text.secondary">Manage items and review claims across the platform</Typography>
            </Box>
            ...
            <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)} maxWidth="xs" fullWidth>
                <DialogTitle>Admin Delete</DialogTitle>
                <DialogContent><Typography>Permanently delete &quot;{deleteTarget?.title}&quot;? This action cannot be undone.</Typography></DialogContent>
                <DialogActions>
