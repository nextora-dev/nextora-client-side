...
import { Box, Grid, Card, CardContent, Stack, Typography, Tabs, Tab, Tooltip, IconButton, alpha, useTheme } from '@mui/material';
...
export type OverviewStat = { label: string; value: number | string; icon: React.ComponentType<{ sx?: object }>; color: string };
...
export default function BoardingHouseCommon({
    title = 'Boarding Houses',
    description = 'Browse and manage boarding house listings',
    overviewStats,
    mainTab,
    onMainTabChange,
    isLoading,
                                    ...
                                    <CardContent sx={{ p: 2 }}>
                                        <Stack direction="row" alignItems="center" spacing={1.5}>
                                            <Box sx={{ width: 40, height: 40, borderRadius: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: alpha(stat.color, 0.1) }}>
                                                <StatIcon sx={{ color: stat.color, fontSize: 20 }} />
                                            </Box>
                                            <Box>
